...
impl State {
    fn new() -> Self {
        ...
        //add ecs
        spawn_player(map_builder.player_start, &mut ecs);
        map_builder.rooms
            .iter()
            .skip(1)
            .for_each(|room| {
                spawn_monster(room.center(), &mut ecs, &mut rng);
            }
        );
...
impl GameState for State {
    fn tick(&mut self, ctx: &mut BTerm) {
        ...
        //turn-based execution of systems
        ...
        //Draw Batch
        render_draw_buffer(ctx).expect("Batch draw error");
    }
}
